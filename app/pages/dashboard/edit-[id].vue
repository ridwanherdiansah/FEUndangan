<template>
  <div class="flex flex-col gap-8 pb-20">
    <header
      class="flex flex-col md:flex-row md:items-center justify-between border-b-4 border-black pb-8 gap-4"
    >
      <div>
        <NuxtLink
          to="/dashboard"
          class="text-[10px] font-black uppercase tracking-[0.2em] hover:underline mb-4 inline-block"
        >
          ← BACK TO PROJECTS
        </NuxtLink>
      ...
      <div class="flex gap-4">
        <button
          class="flex-1 md:flex-none border-2 border-black px-8 py-4 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-black hover:text-white transition-colors"
        >
          SAVE DRAFT
        </button>
        <NuxtLink
          :to="`/${generateSlug(eventData.title)}`"
          target="_blank"
          class="flex-1 md:flex-none bg-black text-white px-8 py-4 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-rose-600 transition-colors text-center inline-flex items-center justify-center"
        >
          PUBLISH
        </NuxtLink>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
      <div class="lg:col-span-5 flex flex-col gap-6">
        <div
          class="bg-black text-white p-4 text-[10px] font-black uppercase tracking-[0.3em] flex justify-between items-center"
        >
        ...
        <section
          v-if="currentSlide === 0"
          class="border-2 border-black p-8 bg-white space-y-6 animate-in"
        >
          <h2 class="text-xl font-black uppercase tracking-tighter border-b-2 border-black pb-2">
            Opening Gate
          </h2>
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest mb-2 text-gray-400"
              >Subtitle</label
            >
            <input
              v-model="eventData.gate.title"
              type="text"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
            />
          </div>
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest mb-2 text-gray-400"
            ...
            >
            <input
              v-model="eventData.gate.openText"
              type="text"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
            />
          </div>
          ...
          <div>
            <label
              class="block text-[10px] font-black uppercase tracking-widest mb-2 text-gray-400 text-rose-500"
              >Background Music (YouTube Link)</label
            >
            <input
              v-model="eventData.youtubeUrl"
              type="url"
        ...
        >
          ...
          <div>
            ...
            >
            <input
              v-model="eventData.cover.headline"
              type="text"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
            />
          </div>
        ...
        >
          ...
          <div>
            ...
            >
            <textarea
              v-model="eventData.quote.text"
              rows="3"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none resize-none"
            ></textarea>
          </div>
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest mb-2 text-gray-400"
            ...
            >
            <input
              v-model="eventData.quote.source"
              type="text"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
            />
          </div>
        ...
        >
          ...
          <div class="grid grid-cols-1 gap-4">
            <input
              v-model="eventData.mempelai.pria"
              placeholder="Groom Name"
              type="text"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
            />
            <input
              v-model="eventData.mempelai.wanita"
        ...
        >
          ...
          <input
            ...
            type="text"
            placeholder="Date (e.g., 12 DEC 2026)"
            class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
          />
          <input
            v-model="eventData.time"
            type="text"
            placeholder="Time (e.g., 19:00 WIB)"
            class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
          />
          <textarea
        ...
        >
          ...
          <p class="text-[10px] font-black tracking-widest uppercase text-gray-500">
            Upload Pre-Wedding Photos
          </p>
          <div class="grid grid-cols-2 gap-4">
            <div
              v-for="(img, idx) in eventData.gallery"
              :key="idx"
              class="border-2 border-black aspect-square bg-gray-100 flex items-center justify-center relative group cursor-pointer overflow-hidden"
            >
              <img v-if="img" :src="img" class="w-full h-full object-cover grayscale" />
              <div v-else class="text-2xl font-black">+</div>
              <div
                v-if="img"
                class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-[10px] font-black tracking-widest transition-opacity"
              >
                CHANGE
              </div>
            </div>
        ...
        >
          ...
          <div>
            ...
            >
            <input
              v-model="eventData.rsvp.deadline"
              type="date"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none"
            />
          </div>
        ...
        >
          ...
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest mb-2 text-gray-400"
              >Mockup Message</label
            >
            <textarea
              v-model="eventData.ucapan.mockMessage"
              rows="2"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none resize-none"
            ></textarea>
          </div>
          <p class="text-[10px] text-gray-500">
            *In real app, this will show a list of comments from database.
          </p>
        </section>
        ...
        >
          ...
          <div>
            ...
            >
            <textarea
              v-model="eventData.closing.text"
              rows="3"
              class="w-full border-2 border-black p-4 text-sm font-bold focus:bg-black focus:text-white outline-none resize-none"
            ></textarea>
          </div>
      ...
      <div class="lg:col-span-7">
        <div class="sticky top-10">
          <div class="flex items-center justify-center w-full h-[80vh] min-h-[500px] py-4">
            <div
              class="relative h-full aspect-[9/16] border-[10px] border-black shadow-[20px_20px_0px_0px_rgba(0,0,0,0.1)] overflow-hidden bg-white"
            >
              <div
                ref="previewContainer"
                class="absolute inset-0 w-full h-full no-scrollbar overflow-y-auto overflow-x-hidden scroll-smooth"
              >
                <component
          ...
          <div class="flex justify-center items-center gap-4 mt-8">
            <button
              ...
              class="border-2 border-black px-4 py-2 font-black disabled:opacity-30"
            >
              PREV
            </button>
...
<script setup lang="ts">
  ...
  const eventData = ref({
    ...
    quote: {
      text: 'AND I KNEW EXACTLY HOW CINDERELLA FELT WHEN SHE FOUND HER PRINCE.',
